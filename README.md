...
│   └── front/                  # Frontend (ClojureScript)
│       ├── core.cljs           # Entry point, роутинг, layout
│       ├── db.cljs             # Re-frame база данных
│       ├── events.cljs         # Re-frame события
│       ├── subs.cljs           # Re-frame подписки
│       ├── websocket.cljs      # WebSocket клиент
│       ├── components/
│       │   └── common.cljs     # Общие UI компоненты
│       └── views/              # Страницы приложения
│           ├── welcome.cljs    # Стартовый экран с документацией
│           ├── dashboard.cljs  # Обзор базы знаний
│           ├── facts.cljs      # Управление фактами
│           ├── rules.cljs      # Управление правилами
│           ├── query.cljs      # Выполнение запросов
│           ├── repl.cljs       # Интерактивный REPL
│           ├── trace.cljs      # Трассировка вывода
│           ├── spy.cljs        # Отладка предикатов
│           ├── puzzles.cljs    # Логические головоломки
│           └── history.cljs    # История транзакций
├── test/logicflow/
│   ├── unify_test.clj
│   ├── core_test.clj
...
Фронтенд построен на **Re-frame** (реактивный фреймворк поверх Reagent):

```
┌─────────────────────────────────────────────────────────────┐
│                        core.cljs                            │
│  (роутинг, layout, sidebar, инициализация)                  │
└─────────────────────────────────────────────────────────────┘
         │                    │                    │
         ▼                    ▼                    ▼
┌─────────────┐      ┌─────────────┐      ┌─────────────┐
│  events.cljs │      │  subs.cljs  │      │   db.cljs   │
│  (события)   │      │ (подписки)  │      │ (состояние) │
└─────────────┘      └─────────────┘      └─────────────┘
         │                    │                    │
         └────────────────────┼────────────────────┘
                              ▼
...
| Страница | Описание |
|----------|----------|
| **Welcome** | Стартовый экран с документацией по языку |
| **Dashboard** | Обзор базы знаний (статистика, графики) |
| **Facts** | Просмотр и добавление фактов |
| **Rules** | Просмотр и добавление правил |
| **Query** | Выполнение запросов к базе знаний |
| **REPL** | Интерактивная консоль |
| **Trace** | Трассировка вывода запросов |
| **Spy** | Отладка конкретных предикатов |
| **Puzzles** | Примеры логических головоломок |
...
- Реализовали DSL для логического программирования с унификацией и backtracking
- Применили метапрограммирование через макросы Clojure для создания Prolog-подобного синтаксиса
- Использовали STM (refs, atoms, agents) для потокобезопасной базы знаний
- Создали веб-визуализатор с real-time обновлениями через WebSocket
- Реализовали трассировку вывода для отладки запросов
...
| Re-frame | События, подписки, состояние UI |
| ClojureScript + Reagent | Компоненты веб-интерфейса |
| WebSocket | Real-time обновления |
| Модульная архитектура | Разделение views, events, subs |
...
- ✅ Welcome экран с документацией по языку
- ✅ Интерактивный REPL
- ✅ Визуализация трассировки вывода
- ✅ Примеры логических головоломок (N-Queens, графы)
